...
export const api = {
  ...
  recordTransaction: (item: { walletId: number; transactionType: string; amount: number; agentNumber: string; clientPhone: string; clientId?: string; clientName?: string }) => request<MnoTransaction>('/mno-transactions', { method: 'POST', body: body(item) }),
